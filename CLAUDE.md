...
**Admin product form: images switched from URL text boxes to real file upload (2026-07-25)** - user pointed out that pasting an external URL for images, and only seeing raw URL text (not an actual preview) in Edit, wasn't the right UX for Add/Edit (same shared `AdminProductForm.vue`, so a fix here covers both). Each image slot is now an "Upload Image" button + a live thumbnail preview instead of a raw URL input; selecting a file uploads it immediately via the new `admin-upload-image` endpoint and stores the returned media-library URL in the same `images` array `admin_save_product()` already expects - no backend change needed there. **Not yet live** - `wp-snippets/admin-upload-image.php` needs to be pasted into WP first (confirmed via curl: currently 404s). Couldn't fully browser-test the upload button itself (clicking a real `<input type=file>` opens a native OS file picker that browser automation can't drive), but the WP endpoint logic and the frontend's FormData/fetch wiring were verified by direct code review - standard, low-risk pattern.
